...
public static class CreateQuery
{
        ...
        string? region)
    {
        return qd =>
        {
            ...
            if (string.IsNullOrWhiteSpace(model) && !string.IsNullOrWhiteSpace(q) && !q.Contains(" ") &&
            ...
            if (!string.IsNullOrWhiteSpace(manufacturer))
                mustQueries.Add(mq =>
                    mq.Term(t => t.Field("manufacturer.keyword").Value(manufacturer.ToLowerInvariant())));
            ...
            if (!string.IsNullOrWhiteSpace(transmission))
                mustQueries.Add(mq =>
                    mq.Term(t => t.Field("transmission.keyword").Value(transmission.ToLowerInvariant())));
            ...
            if (effectiveMinPrice.HasValue || effectiveMaxPrice.HasValue)
                ...
                {
                    nr.Field(f => f.Price);
                    if (effectiveMinPrice.HasValue) nr.Gte((double)effectiveMinPrice.Value);
                    if (effectiveMaxPrice.HasValue) nr.Lte((double)effectiveMaxPrice.Value);
                })));
            ...
            if (effectiveMinYear.HasValue || effectiveMaxYear.HasValue)
                ...
                {
                    nr.Field(f => f.Year);
                    if (effectiveMinYear.HasValue) nr.Gte(effectiveMinYear.Value);
                    if (effectiveMaxYear.HasValue) nr.Lte(effectiveMaxYear.Value);
                })));
            ...
            if (odometerFrom.HasValue || odometerTo.HasValue)
                mustQueries.Add(mq => mq.Range(r => r.Number(nr =>
                {
                    ...
                    if (odometerFrom.HasValue) nr.Gte(odometerFrom.Value);
                    if (odometerTo.HasValue) nr.Lte(odometerTo.Value);
                })));
