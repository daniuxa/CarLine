...
{
    public static IServiceCollection AddCarLineBlobStorage(this IServiceCollection services,
        IConfiguration configuration, BlobStorageOptions? options = null)
    {
        var opts = options ?? new BlobStorageOptions();
        ...
        services.AddSingleton(sp =>
        {
            ...
            if (opts.EnsureContainerExists)
                ...
                {
                    logger.LogWarning(ex, "Failed to ensure blob container '{container}' at startup.",
                        opts.ContainerName);
                }
        ...
        BlobStorageOptions options)
    {
        foreach (var key in options.ConfigurationKeys)
        {
            ...
            if (!string.IsNullOrWhiteSpace(value)) return (value, $"config:{key}");
        }
        ...
        foreach (var env in options.EnvironmentVariables)
        {
            ...
            if (!string.IsNullOrWhiteSpace(value)) return (value, $"env:{env}");
        }
