...
// ── SRP: this module owns exactly one concern ─────────────────────────────────
// Upload a local file to Azure Blob Storage with exponential-backoff retry.
// Extracted from DataCleanupService to follow the Single Responsibility Principle.
...
/// Handles slow antivirus scanners and delayed handle release on Windows.
/// Recursive 'let rec' replaces the C# for-loop with Math.Pow.
...
/// Uploads a local file to blob storage and returns the resulting blob name.
