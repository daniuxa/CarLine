...
//
// The C# version is 168 lines of imperative if/return false chains.
// Here the entire validation pipeline is expressed as a single |> chain.
//
// Each step receives the (csvRecord, fullRecord) pair and returns:
//   Ok (csv, full)  → record passes this step, continue
//   Error reason    → record is invalid, short-circuit the whole pipeline
//
// Result.bind threads Ok values forward and skips Error values automatically.
...
// ── Reusable: validate one categorical field against an allowed set ────────────
let private validateField fieldName (validValues: Collections.Generic.HashSet<string>) ((csv, full): Records) =
...
let private normalizeModel ((csv, full): Records) =
    ...
    | true, model when not (String.IsNullOrWhiteSpace(model)) ->
        // Take the first word only — Array.tryHead returns 'string option', no null possible
        let firstWord =
...
let private normalizeRegion ((csv, full): Records) =
    ...
    | true, region when not (String.IsNullOrWhiteSpace(region)) ->
        ...
        full["region"] <- norm
    | _ -> ()
    Ok (csv, full)  // region is optional — always succeeds

// ── Public API ────────────────────────────────────────────────────────────────
/// Returns true if the record is valid and cleaned; false if it should be dropped.
