    ...
    : ControllerBase
{
    ...
    {
        if (string.IsNullOrWhiteSpace(request.Manufacturer) || string.IsNullOrWhiteSpace(request.Model) || request.Year <= 0 || request.Odometer < 0)
        {
        ...
        try
        {
            ...
            return Ok(new { predictedPrice = Math.Round(prediction.Score, 2), input = request, timestamp = DateTime.UtcNow });
        }
        catch (InvalidOperationException ex)
        {
            logger.LogWarning(ex, "Model not available");
            return Problem(detail: ex.Message, statusCode: 503, title: "Model Not Available");
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Prediction failed");
            return Problem(detail: ex.Message, statusCode: 500, title: "Prediction Failed");
        }
    }
    ...
    public async Task<IActionResult> PredictBatch([FromBody] List<CarPredictionRequest> requests)
    {
        if (requests.Count == 0)
        {
            return BadRequest(new { error = "Request batch cannot be empty." });
        }
        ...
        catch (Exception ex)
        {
            ...
            return Problem(detail: ex.Message, statusCode: 500, title: "Batch Prediction Failed");
        }
    }
}
